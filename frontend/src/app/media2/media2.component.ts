...
import { Media2Service } from '../services/media2.service';
...
@Component({
  ...
  styleUrl: './media2.component.css'
})
export class Media2Component {

  constructor(
    private route: ActivatedRoute,
    private mediaService: Media2Service,
  ...
  ngOnInit(): void {
    ...
    this.mediaService.fetchAllMedia(this.moviesOnly()).subscribe(
      (mediaList) => {
  ...
  computeFilteredNSortedMedia(): Media[] {
    ...
    console.log("C: Last update time:", this.lastUpdateTime);
  ...
  fetchUpdatedMedia(forceUpdate: boolean = false): void {
    ...
    if (delta > this.UPDATE_INTERVAL || forceUpdate) {
      ...
      this.mediaService.fetchUpdatedMedia(delta).subscribe(
        (mediaList) => {
          console.log("C: Updated media fetched");
