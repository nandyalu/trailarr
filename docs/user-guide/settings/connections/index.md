...
- **Plex libraries you do not monitor** {{ version_badge("add", "0.12.1") }} — a Plex connection reads only the libraries that have a library folder row, so the doctor leaves the other libraries out. It lists them as skipped, and does not ask you to add a mapping for them. To monitor one of them, add a library folder row for it. A Plex connection with no library folder rows at all is checked in full, because it has made no choice yet.
