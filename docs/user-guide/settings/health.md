...
Trailarr asks YouTube for a search result rather than a fixed video. A fixed video can be deleted — the one this test used before `v0.12.1` was, and every health page then reported that the setup was broken when nothing was wrong. A search always resolves to a video that exists, and it uses the same code that Trailarr uses when it searches for a trailer.
