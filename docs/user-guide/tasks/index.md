...
- {{ version_badge("upd", "0.11.5") }} The task scans the library in sweeps, so media and profiles that become eligible during a long run can still be downloaded in that run; profile changes apply to downloads that have not started, while trailers already downloaded keep the profile settings used to download them.
