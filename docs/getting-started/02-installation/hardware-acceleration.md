...
!!! tip "Check which GPUs Trailarr detected"
    {{ version_badge("add", "0.11.4") }}
    [Settings > Health](../../user-guide/settings/health.md) shows which GPUs Trailarr detected at startup, and whether each GPU is enabled. You do not need to read the logs.
