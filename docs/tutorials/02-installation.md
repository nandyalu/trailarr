...
Before you begin, make sure you have reviewed the [First Things](./01-first-things.md) section to understand environment variables and how to find your existing Radarr/Sonarr volume mappings.
...
- **Media Folders:**
    ...
    *   The right side (`:/movies`, `:/tv`) is how this path will be known *inside the Trailarr container*. You will use these paths later when configuring connections in Trailarr.
    ...
    Remember that we got our volumes from `Radarr` and `Sonarr` Docker compose files in [First Things](./01-first-things.md), we need to use them here!
    ...
    ??? example "Example 3 - Different `Radarr` and `Sonarr` folders mapped to same container folder"
        ...
        So instead of mapping both to same internal folder `/data`, what we do is change them a little bit, but we need to remember these for configuring [Path Mappings](./05-connections.md#connections) later!
...
Now that Trailarr is up and running, we need to configure it to connect to our `Radarr` and `Sonarr` to let it start doing it's job - download trailers. We will go over that in the next section [Configuration](./03-configuration.md).
...
For a detailed explanation of how to setup each of these see [Docker Compose](#docker-compose).

*   `-d`: Runs the container in detached mode (in the background).
*   `--name trailarr`: Assigns a name to the container.
*   `-e OPTION=VALUE`: An environment variable to use with the container.
*   `-p 7889:7889`: Maps port 3699 on your host to port 3699 in the container. Change the host port (left side) if needed.
