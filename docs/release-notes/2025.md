...
- New downloads are now automatically tracked and added to the database.
- The `Scan Disk for Trailers` task has been updated to analyze existing files and add them to the database if they are not already tracked.
- A new `Downloads` section has been added to the `Media Details` page to display all the downloaded trailers for the selected media.
- A refresh button has been added to the `Downloads` section to rescan the media folder for any new trailers.
- Added a new API endpoint to scan and refresh the downloads for a media item.

**Bug Fixes:** 🐛

- Fixed a bug that caused the app to crash when generating the OpenAPI files.
