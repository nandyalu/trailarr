## **v0.6.4-beta** - _December 28, 2025_

**What's New:** ✨

- Improved wait time and error handling in `Download Missing Trailers` task to catch all exceptions per media item, ensuring that processing continues even if one item fails. Added logging for unexpected errors.
- Added `Downloaded` as a sort option in Media Library pages.

**Bug Fixes:** 🐛

- Fixed a bug that caused `Download Missing Trailers` task to skip processing on exceptions.
- Fixed a bug that prevented `season_count` to be added as a filter option.
- Fixed a bug that caused `Audio Volume Level` to be applied for certain conversions even when not enabled.

**Other Changes:** ⚡

- Updated FastAPI to `0.127.0` and various python packages.
- Updated various npm packages.
- Refactored `Filters Dialog` component to use signal forms.


## **v0.6.3-beta** - _December 14, 2025_
...
- Added options to delete the downloaded trailers when media file is deleted from disk, and/or when media is removed from the connected Arr application. These options can be enabled from `Settings > General > Delete Trailers`. See [General Settings](https://nandyalu.github.io/trailarr/user-guide/settings/general-settings/#delete-trailer-settings){:target="_blank"} for more details. Thanks to PR [#414](https://github.com/nandyalu/trailarr/pull/414){:target="_blank"} by [@lems111](https://github.com/lems111){:target="_blank"}.
...
- Added an example configuration for setting up Network Drives with Docker Compose in the [Getting Started](https://nandyalu.github.io/trailarr/getting-started/02-installation/docker-compose/#network-drives){:target="_blank"} section of the documentation.
