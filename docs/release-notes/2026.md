...
- **New `Create Missing Folders` setting** — Trailarr can now create a media folder that does not exist instead of skipping the download, which helps when you keep trailers in a folder of their own; it is off by default and never creates folders on unreachable storage — see [Create Missing Folders](../user-guide/settings/general-settings/index.md#create-missing-folders). ([#641](https://github.com/nandyalu/trailarr/discussions/641))
...
- **Dates now show in your own timezone** — every date in the web interface was printed in UTC, so a trailer added at 9 AM in Melbourne showed as the evening before; dates now follow the timezone of the device you view them on. ([#650](https://github.com/nandyalu/trailarr/issues/650))
