...
- **A show with a stray duplicate folder can no longer take over the whole library** — when a Plex show has episodes in more than one folder, Trailarr now selects the folder with the most episodes instead of storing the library root, which made one media item claim every file and trailer under it — see [Plex Connection](../user-guide/settings/connections/plex.md#how-trailarr-resolves-tv-show-folder-paths).

- **Media rows with a library-root folder are never matched again** — a bad row left behind by the bug above could adopt the next new series added from Radarr/Sonarr; such rows are now excluded from all folder matching, so the next Plex sync cleans them up automatically.

**Other Changes:** ⚙️

- Plex items now link to their Radarr/Sonarr media by TMDB/TVDB id when the folder paths do not match — see [Plex Connection](../user-guide/settings/connections/plex.md#how-plex-items-link-to-media).

- A Plex-only media item now keeps its history when its folder is renamed in Plex; before, the item was deleted and added again.
