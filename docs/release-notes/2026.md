...
- **Media status is now live-computed, never stored-then-trusted** — status is always derived from your actual downloads and the monitor flag, so it can never get stuck or drift out of sync with reality — see [Filtering](../user-guide/library/index.md#filtering).

- **"Downloading" can never get stuck again** — in-progress downloads are tracked in memory and pushed live over the websocket, so a crash or restart mid-download can never leave items showing *Downloading* forever.

- **Per-profile download matrix on Media Details** — a new [Download Profiles](../user-guide/library/media-details/index.md#download-profiles-section) section shows exactly where every profile stands with a media item (not matching, satisfied, pending, or backing off), computed with the same rule the download engine uses.

- **Preview mode — see what Trailarr *would* download before it downloads anything** — turn off the new [Downloads Enabled](../user-guide/settings/general-settings/index.md#downloads-enabled) setting and the download task publishes its full work list without downloading anything, shown as a *Preview mode* banner on the media pages. ([#36](https://github.com/nandyalu/trailarr/issues/36), [#435](https://github.com/nandyalu/trailarr/issues/435))

- **Monitoring is yours now** — the monitor flag is written only by you (plus once at creation, from the connection's new [Monitor New Media](../user-guide/settings/connections/index.md#monitor-types) toggle), and Radarr/Sonarr/Plex syncs and downloads never change it again.

> ⚠️ **Using Monitor Type "Sync"?** Trailarr no longer follows monitoring changes from Radarr/Sonarr — to keep sync-like behavior, add a profile filter on `arr_monitored`, see [Sync-like behavior with `arr_monitored`](../user-guide/settings/profiles/filters.md#sync-like-behavior-with-arr-monitored).

> ⚠️ **Stop Monitoring removed** — the deprecated per-profile option is gone, and if several profiles match the same media each now downloads its own video — see [Stop Monitoring](../user-guide/settings/profiles/settings/general.md#stop-monitoring).

- **Downloads never target unreachable storage** — Before downloading, Trailarr now verifies that the storage backing the media folder is actually reachable and readable. A disconnected network drive is skipped with a clear *Storage unreachable* reason (and retried next run) instead of being treated like a missing folder — and a half-dead mount can no longer cause a failed write that counted as a download failure and triggered the retry backoff.

**Bug Fixes:** 🐛

- **Discord notifications now show a useful preview on your phone** — mobile push notifications read the media title and event (e.g. *Inception (2010) ⬇️ Trailer Downloaded*) instead of *"image received"*, with posters for Radarr/Sonarr media loaded from their public TMDB/TVDB image URLs like other Arr apps do.

**Other Changes:** 🔧

- Notifications are now sent within ~10 seconds instead of 30, capped at 5 messages per minute — a bulk operation simply merges into fewer, bigger summary messages.
- The `SIGNAL-DISAGREE` bake-window logging from v0.10.0 has been removed — the per-profile matrix explains every decision on demand.
