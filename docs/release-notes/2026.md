...
- **The Health page no longer reports that every setup is broken** — the YouTube download test asked for one particular video, and that video was deleted. Trailarr now asks for a search result, which always exists. A video that cannot be read gives a warning that says so, instead of an error about your setup — see [Health](../user-guide/settings/health.md#youtube-download-test).
