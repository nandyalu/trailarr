## **v0.9.1-beta** - _May 4, 2026_

**Bug Fixes:** 🐛

- Fixed a `PermissionError` on container startup when `URL_BASE` is set — `index.html` is now made world-writable at build time so `appuser` can apply the `<base href>` patch at runtime.
