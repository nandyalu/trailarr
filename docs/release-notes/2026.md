...
- **The missing-trailer task no longer gets progressively slower in a large library** — the task now resumes its database scan after the last media item that it examined. Before this change, it started at the first media item again for each trailer and could use high CPU for weeks.
