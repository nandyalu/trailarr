...
- **The image refresh and the library sync work again** — both tasks stopped with an error as soon as they reached a media item, because a log line used a feature of a logger that this file does not have. The `Image Refresh` task failed every run, and `Arr Data Refresh` failed at the end, where it refreshes the images of the media it just synced.
