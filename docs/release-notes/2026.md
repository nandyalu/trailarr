...
- **Media status is now live-computed, never stored-then-trusted** — What you see is always derived from your actual downloads and the monitor flag: any active download → *Downloaded*, else monitored → *Monitored*, else *Missing*. Status can no longer get stuck or drift out of sync with reality, and the built-in filters (Downloaded / Missing / Unmonitored), home-page stats and "recently downloaded" all decide from download records now instead of legacy flags.

- **"Downloading" can never get stuck again** — In-progress downloads are tracked in memory and pushed live over the websocket instead of being written to the database. If the app crashes or restarts mid-download, nothing is ever left showing *Downloading* forever — the state simply resets by construction.

- **Per-profile download matrix on Media Details** — A new **Download Profiles** section shows, for every profile: whether it matches this item, whether it's already satisfied (by its own download, by claiming an existing file, or covered by a stop-monitoring profile), or whether it's pending / backing off after failures (with attempt count, last error and next retry time). It's computed with the exact same rule the download task uses, so what you see is what the engine will do.

- **Preview mode — see what Trailarr *would* download before it downloads anything** — A new **Downloads Enabled** setting under Settings → General. When disabled, the scheduled download task computes and publishes its full work list but downloads nothing; the media pages show a *Preview mode* banner with the would-download count and a list of every (media, profile) pair. Scans, syncs and attribution keep running so the preview stays accurate, and manual downloads still work. Great for pointing Trailarr at a large, carefully curated library without a leap of faith. ([#36](https://github.com/nandyalu/trailarr/issues/36), [#435](https://github.com/nandyalu/trailarr/issues/435))

- **Monitoring is yours now** — The monitor flag is written only by you (and once, when media is first added). Radarr/Sonarr/Plex syncs **never change it again**: unmonitor a movie and it stays unmonitored forever, no matter what your Arr apps do. The monitor toggle also lost its old guardrails — you can monitor anything, trailer or not (the download engine already knows what's satisfied). The per-connection **Monitor Type** dropdown becomes a simple **"Monitor New Media"** Yes/No toggle: it only decides the starting value for newly added media.

> ⚠️ **Using Monitor Type "Sync"?** Your existing media keep their current monitor values, but Trailarr will no longer follow monitoring changes from Radarr/Sonarr. To keep sync-like behavior, add a filter **`arr_monitored` `equals` `true`** to your download profiles — Trailarr keeps updating `arr_monitored` from your Arr apps on every sync, so profiles filtered on it naturally track what's monitored in Arr. The upgrade logs list your affected connections.

> ⚠️ **Stop Monitoring removed** — the deprecated per-profile option is gone (downloads are tracked per profile since v0.10.0, so it was no longer needed to prevent re-downloads). Note for overlapping-profile setups: if several of your profiles match the same media, each now gets its own download (previously a Stop-Monitoring profile's download suppressed the others). Non-overlapping setups — like the default Movie/Series profiles — see no change.

**Other Changes:** 🔧

- The `SIGNAL-DISAGREE` bake-window logging from v0.10.0 has been removed — the new per-profile matrix explains every decision on demand.
- New API endpoints: `GET /api/v1/media/{id}/pending` (per-profile matrix), `GET /api/v1/media/pending` (library-wide work list) and `GET /api/v1/media/downloading` (in-flight downloads).
- **API break:** connection objects now carry `monitor_new_media: bool` instead of `monitor: <enum>` — API consumers creating or updating connections must send the new field.
