...
- **Notifications via Apprise** — Add your favorite services under **Settings → Notifications**: Discord, Telegram, Slack, email and [100+ more](https://appriseit.com/services/). Each channel picks which events it receives (e.g. a `#trailarr-downloads` channel subscribed to *Trailer Downloaded*). Bulk operations are batched into a single summarized message, so a library-wide sync won't flood your channel. Apprise URLs are stored write-only and never shown again after saving. A **Test** button verifies the setup instantly.
