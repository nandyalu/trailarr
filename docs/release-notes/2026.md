...
- Fixed `install.sh` crashing with `EOFError` on Linux when run via `curl | sh`. The shell's stdin is the pipe carrying the script, so interactive prompts had no terminal to read from. The installer now redirects `/dev/tty` for the Python process so port prompts work correctly.
