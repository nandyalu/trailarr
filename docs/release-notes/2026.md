...
- **Trailers now come from the list that TMDB curates** — add a TMDB API key in `Settings > General`, and Trailarr downloads a trailer that the studio published instead of searching YouTube and taking the best match. Trailarr works the same as before without a key — see [TMDB](../user-guide/settings/tmdb.md).
...
- **`Always Search` has a narrower meaning** — it now means "do not reuse what a search found before". It keeps the video you chose, the TMDB list, and the id from Radarr or Sonarr, which it used to throw away.
...
- **Sonarr does report a YouTube trailer id** — the documentation said that it does not, and Trailarr has always read it. The FAQ and the media details page are corrected.
