...
- **A video you choose is now kept** — paste a YouTube link on the media details page, and that video goes to the top of the list. No task removes it, not even when a download of it fails. Before, an automatic search could replace the link that you set.

- **Pick the language of the trailer** — a profile has a new `Trailer Language`. TMDB lists a trailer per language, and Trailarr prefers the one you ask for. It is a preference and not a filter: a trailer in another language is still downloaded when the language you ask for has none — see [Trailer Language](../user-guide/settings/profiles/settings/general.md#trailer-language).
...
- **`Always Search` keeps two things it used to throw away** — it still ignores the id that Radarr reports and the result of an earlier search, which is why you turn it on. It now keeps a video you chose on the media details page, and the trailers that TMDB lists, so a profile that searched for everything takes a curated trailer in your language first.
