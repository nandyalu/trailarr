...
- **Direct installs now report the correct version after updating** — Running `trailarr update` on a bare-metal (non-Docker) install replaced the application files but never updated the recorded version, so the UI and startup banner kept showing the old version even though the update itself succeeded. The update command now writes the new version alongside the app files. Note: since the update runs the previously installed CLI, the version display corrects itself starting with the *next* update after this release — or immediately, by editing `APP_VERSION` in the data directory's `.env` (e.g. `/var/lib/trailarr/.env` on Linux) and running `trailarr restart`.
