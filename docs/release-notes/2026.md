...
- **The log database no longer keeps a large leftover file** — the `logs.db-wal` file in the `logs` folder stayed as large as the biggest daily log cleanup, often hundreds of MB, and a restart did not shrink it; Trailarr now empties it after each cleanup and at each start and stop, so the first start after the update gives the space back. Thanks [@Jorman](https://github.com/Jorman) for the report and the file sizes that showed it! ([#687](https://github.com/nandyalu/trailarr/issues/687))

- **A wrong API key now says so** — when Radarr or Sonarr refused the API key, Trailarr said `Unable to connect to API. Check your connection.`; it now names the problem, such as `Unauthorized. Check that the API key for this connection is correct.`

- **A batch delete reports each item once** — when a trailer could not be deleted, you saw two error messages and the rest of the batch stopped; Trailarr now reports that item once and goes on with the others.

---

## **v0.13.0** - _September 24, 2026_

**What's New:** ✨

- **Trailers now come from the list that TMDB curates, in the language you want** — add a TMDB API key in `Settings > General`, and Trailarr downloads a trailer that the studio published instead of searching YouTube and taking the best match. Radarr reports one trailer id and it is usually English, so a trailer in another language meant turning on `Always Search` and accepting whatever a search returned. Trailarr now asks TMDB for a trailer in the language your profile asks for, and searches only when there is none — see [TMDB](../user-guide/settings/tmdb.md). Thanks [@Jorman](https://github.com/Jorman) for the request, and for the TMDB queries that showed how! ([#511](https://github.com/nandyalu/trailarr/issues/511))

- **Every media item shows the videos Trailarr can download** — the new `Known videos` list on the media details page shows each video and where it came from: one you chose, the TMDB list, the id from Radarr or Sonarr, or an earlier YouTube search. Trailarr takes the first one that works — see [Known videos](../user-guide/library/media-details/index.md#known-videos).
...
- **Pick the language of the trailer** — a profile has a new `Trailer Language`. TMDB lists a trailer per language, and a profile that names one downloads that language or nothing: Trailarr never gives you a German trailer when you asked for Italian. When TMDB has no trailer in your language, Trailarr searches YouTube with your search query and says so in the log. Leave the setting empty, which is the default, and nothing changes — see [Trailer Language](../user-guide/settings/profiles/settings/general.md#trailer-language).
...
- **The filter button counts what it found** — the button now reads `2789 / 3746`: how many items the filter matched, and how many the page can show. On the `Home` page this is how much of your library has a trailer. On the `Movies` and `Series` pages, select `Downloaded` or `Missing` to see the same count for that type — see [Filtering](../user-guide/library/index.md#filtering). Thanks [@jaycedk](https://github.com/jaycedk) for the idea and the exact format! ([#618](https://github.com/nandyalu/trailarr/issues/618))

**Other Changes:** ⚙️

- **Old database backups are deleted** — Trailarr copies the database at every start, and kept the newest 30 copies. That limits a count, not a disk: one user found 1.5 GB of backups. Trailarr now keeps 10 backups and deletes any backup more than 30 days old. Set `BACKUP_KEEP_COUNT` and `BACKUP_KEEP_DAYS` to change the limits — see [Environment variables](../getting-started/01-first-things/environment-variables.md#backup_keep_count-and-backup_keep_days). The copy from the current start is never deleted, and the `update_<version>` folders of a direct installation now obey the same limits. Thanks [@JacobBrown4](https://github.com/JacobBrown4) for the report, and for the file sizes that showed the cause! ([#681](https://github.com/nandyalu/trailarr/issues/681))

- **The Health page tells you about the TMDB key** — the page lists the key with the rest of your setup, says what Trailarr does without one, and links to the page that says how to get one. It stays grey while no key is set: a key is optional, so the page does not report an issue for it — see [Health](../user-guide/settings/health.md#the-checks).
...
- **`Always Search` ignores the TMDB list too** — the setting means what it says: no id from any source, a YouTube search every time. If you turned it on to get a trailer in your language, a TMDB key with a `Trailer Language` does that better, so turn it off and set the language — see [Always Search](../user-guide/settings/profiles/settings/search.md#always-search).

- **The API has three new endpoints** — `GET`, `POST` and `DELETE` on `/api/v1/media/{id}/videos` read, add and remove the known videos of a media item.
...
## **v0.12.1** - _September 12, 2026_

**Bug Fixes:** 🐛

- **The image refresh and the library sync work again** — both tasks stopped with an error as soon as they reached a media item, because a log line used a feature of a logger that this file does not have. The `Image Refresh` task failed every run, and `Arr Data Refresh` failed at the end, where it refreshes the images of the media it just synced. Thanks [@St3ffn](https://github.com/St3ffn) for the detailed report, which named the file and the cause! ([#677](https://github.com/nandyalu/trailarr/issues/677))

- **A dialog opens again in Safari** — a dialog showed nothing: the page went grey behind it, but the box had a height of zero. A dialog takes its height from its contents, and Safari measures a column part that starts from a percentage as zero tall. Each part of a dialog now starts from the height of its own content. This applies to every dialog, such as Custom Filters and the profile selection for a download. Thanks [@carlosjfcasero](https://github.com/carlosjfcasero) for the report and for the measurements from the browser console! ([#653](https://github.com/nandyalu/trailarr/issues/653))

- **A Plex library that you do not monitor is no longer reported as a problem** — the Connection Doctor checked every library that Plex reports, so the libraries you removed from the connection came back as folders that Trailarr cannot see, each with a path mapping to add. The doctor now skips them, and says why — see [Connection Doctor](../user-guide/settings/connections/index.md#connection-doctor).

- **The disk report names a disk, not one film** — the Health page listed a folder such as `'/media/movies/all/Varavu (2026) {imdb-tt37963237}': 3.9 TiB free`. The free space was right, but the name was the folder of one title, which reads like a report about that title. The page now names your library folders, and falls back to the disk the media sits on.

- **The Health page no longer reports that every setup is broken** — the YouTube download test asked for one particular video, and that video was deleted, so every setup reported a fault. The test no longer depends on one id: it searches for a trailer and reads the first result. When that video cannot be read, Trailarr tries a second search, and then says the video is the problem instead of reporting an error about your setup — see [Health](../user-guide/settings/health.md#youtube-download-test).

- **The theme and the preview banner show on the first page load** — the web page asked for your settings before it had a session, got a refusal, and kept it for the rest of the visit. The theme stayed on `Auto` and the preview-mode banner did not show while downloads were off.

**Other Changes:** ⚙️

- **A burst of events no longer floods your notification channel** — a job that makes thousands of events, such as a library scan after you change a path mapping, sent one message every ten seconds. Trailarr now waits longer after a big batch, up to five minutes, and goes back to ten seconds as soon as the job stops. A scan that finds 1,292 renamed trailers sends about four messages instead of about forty, and each message says how many of each kind there are, such as `Trailer Renamed — 1,292 items` — see [Notifications](../user-guide/settings/notifications.md#good-to-know).

- **A failed connection update gives the correct status** — the API answered `404` when your Radarr, Sonarr or Plex server refused the request or did not answer, which is the same answer as a connection that does not exist. The API now answers `400` for a problem with the server, and keeps `404` for an id that is not in the database. The message that you see does not change.

---

## **v0.12.0** - _September 10, 2026_

This release moves the backend code into layers. It adds no features, and Trailarr behaves the same. It is Phase 7 of the [roadmap](../references/roadmap.md).

**Bug Fixes:** 🐛

- **An error answer no longer shows internal details** — eighteen API endpoints answered every failure with the status `404` and the text of the exception. That text can hold a folder path, a database statement, or a web address with a token in it. Each failure now gets the correct status, and a failure that Trailarr cannot explain to you gives a short line that names only the action. The full error goes to the log. A message that helps you, such as `Connection refused` from a connection test, stays as it is. A failed Radarr or Sonarr request also put the API key of that connection into its message. The message now tells you to check the key, and it does not show the key.
...
- **The backend code is in five layers** — `api`, `services`, `database`, `tasks` and `utils`. Your database, your settings and the API answers do not change. The new structure makes the next phases of the [roadmap](../references/roadmap.md) safer to build.

- **The API documentation lists the errors of each endpoint** — the OpenAPI specification now shows the `500` response of eighteen endpoints, and the `404` response of two more endpoints that always could give one.
...
## **v0.11.5** - _September 3, 2026_

**Bug Fixes:** 🐛

- **The missing-trailer task no longer gets slower as it runs** — the task read the whole library again for every trailer that it downloaded. On a large library it used high CPU for hours. The task now reads the library in sweeps, and it looks up the download attempts in one batch. A profile that you change during a run still applies to every item that has not started; see [Tasks](../user-guide/tasks/index.md#download-missing-trailers). Thanks [@sittingmongoose](https://github.com/sittingmongoose) for finding the cause and for the profiling that showed it! ([#666](https://github.com/nandyalu/trailarr/pull/666))
...
- **New Settings > Health page** — the page shows live checks of FFmpeg, hardware acceleration (finally visible!), yt-dlp, cookies, connections, the image cache, and disk space. Each check that fails names its fix — see [Health](../user-guide/settings/health.md).

- **Set up YouTube cookies from the UI** — upload or paste your cookies.txt on the Health page. Trailarr stores the file with safe permissions, and shows whether the cookies are valid or expired. Trailarr never shows the content again — see [YouTube cookies](../user-guide/settings/health.md#youtube-cookies).
...
- **Modals show again in Safari** — a dialog stayed hidden while the page behind it dimmed. A dialog now shows even when its opening animation does not run. Trailarr also sets the position and the size limits of a dialog against the window. Before this change, Safari could measure the dialog against a short parent element and give it a height of 0. Thanks [@carlosjfcasero](https://github.com/carlosjfcasero) for the report and the console output! ([#653](https://github.com/nandyalu/trailarr/issues/653))

- **Login works again behind a reverse proxy sub-directory** — the login returned a `405 Method Not Allowed` error, or was successful and then gave a `401 Unauthorized` error for every request, which locked you out of the app; the session cookie now stays at the root path, the redirect after login keeps the URL base, and a new `URL Base` applies without a restart — see [Reverse Proxy](../user-guide/reverse-proxy.md#login-problems-in-a-sub-directory-setup). Thanks [@daviddavo](https://github.com/daviddavo) for the detailed report and logs! ([#663](https://github.com/nandyalu/trailarr/issues/663))

- **Trailarr uses your environment variables again** — a setting stored in the `.env` file replaced the variable that you set in `docker-compose.yml`. So `URL_BASE=` and `WEBUI_PASSWORD=` did nothing, and you could not correct a setting that locked you out. A variable that you set for the container now has priority — see [Environment Variables](../getting-started/01-first-things/environment-variables.md) ([#663](https://github.com/nandyalu/trailarr/issues/663)).

**Other Changes:** ⚙️

- `WEBUI_PASSWORD` with a value that is not empty now sets that password, and Trailarr stores only its hash. Before this change, such a value broke the login. An empty value still resets the password to `trailarr` — see [WEBUI_PASSWORD](../getting-started/01-first-things/environment-variables.md#webui_password).

- Trailarr now records a known yt-dlp failure as a plain-language reason with the fix (for example, "YouTube requires a sign-in for this download. Set up a cookies file on Settings > Health.") instead of a raw traceback — see [Classified download errors](../user-guide/settings/health.md#classified-download-errors).
...
## **v0.11.3** - _August 21, 2026_

**What's New:** ✨

- **Filter your library by downloads and files** — view filters gain a new **Downloads** field family: `Download Count`, `Download Profile`, `Download Resolution`, `Download Added At`, `Download File Missing`, and `Has Unknown Profile Download` — see [Download Filters](../user-guide/settings/profiles/filters.md#download-filters-view-filters-only).

- **Recipe: find trailers below 1080p** — combine `Has Downloads Equals true` with `Download Resolution Less Than 1080` in a view filter to list media whose trailer needs a re-download — see [this example](../user-guide/settings/profiles/examples.md#example-4-missing-1080p-trailer-view-filter).
...
- **Downloaded trailers show in Plex immediately again** — the `Notify Plex` profile setting scanned the media folder, but Plex kept the new trailer hidden on the media item until its metadata refreshed. Trailarr now refreshes the item too, so the trailer shows in seconds — see [Notify Plex](../user-guide/settings/profiles/settings/plex.md#notify-plex). Thanks [@lems111](https://github.com/lems111) for finding the cause and the fix! ([#652](https://github.com/nandyalu/trailarr/pull/652))
...
- **Trailer Profiles now reject download filters (intentional change)** — a profile that filters on its own downloads is circular: it stops matching a media item the moment its trailer downloads. The download engine already prevents re-downloads, so profiles do not need these filters. This includes `Has Downloads`, which profiles accepted before; saving such a profile now shows an error that recommends a view filter with the [Download Filters](../user-guide/settings/profiles/filters.md#download-filters-view-filters-only) instead. Existing profiles keep working until you edit them.

---

## **v0.11.2** - _August 15, 2026_

**What's New:** ✨

- **New `Create Missing Folders` setting** — Trailarr can create a media folder that does not exist, instead of skipping the download. This helps when you keep trailers in a folder of their own. The setting is off by default. Trailarr never creates a folder when the drive or network share is unreachable — see [Create Missing Folders](../user-guide/settings/general-settings/index.md#create-missing-folders). ([#641](https://github.com/nandyalu/trailarr/discussions/641))

**Bug Fixes:** 🐛

- **A show with a stray duplicate folder no longer takes over the library** — when a Plex show has episodes in more than one folder, Trailarr selects the folder with the most episodes. Before, Trailarr stored the library root, and that one media item claimed every file and trailer below it — see [Plex Connection](../user-guide/settings/connections/plex.md#how-trailarr-resolves-tv-show-folder-paths).

- **A media item with a library-root folder is never matched again** — a bad item from the problem above could take over the next new series from Radarr/Sonarr. Trailarr now ignores these items in all folder matches. The next Plex sync deletes them.

- **Dates show in your own timezone** — the web interface showed all dates in UTC. A trailer added at 9 AM in Melbourne showed as the evening before. Dates now use the timezone of the device that you use — see [#650](https://github.com/nandyalu/trailarr/issues/650).

**Other Changes:** ⚙️

- Plex items link to their Radarr/Sonarr media by TMDB/TVDB id when the folder paths are different — see [Plex Connection](../user-guide/settings/connections/plex.md#how-plex-items-link-to-media).

- A Plex-only media item keeps its history when you rename its folder in Plex. Before, Trailarr deleted the item and added it again.

---

## **v0.11.1** - _August 11, 2026_

**What's New:** ✨

- **You control AI subtitles now** — the new per-profile `Auto-Generated Subtitles` setting (off by default) decides if YouTube auto-generated subtitles are used when a video has no uploader subtitles — see [Subtitle Settings](../user-guide/settings/profiles/settings/subtitle.md#auto-generated-subtitles). ([#644](https://github.com/nandyalu/trailarr/issues/644))

**Bug Fixes:** 🐛

- **Plex-only media no longer lingers after deletion** — when an item is removed from your Plex library and has no Radarr/Sonarr link, the next Plex sync now deletes it from Trailarr, the same way Arr syncs already clean up — see [Plex Connection](../user-guide/settings/connections/plex.md#what-a-plex-connection-does).
...
- **YouTube downloads work on direct installs** — the installer now bundles the [Deno](https://deno.com) JavaScript runtime that `yt-dlp` needs to solve YouTube's challenges; without it, every download failed with `Requested format is not available` — see [Self Install](../getting-started/02-installation/self-install.md#deno-javascript-runtime-for-yt-dlp).
...
- Trailarr checks the configured `yt-dlp`, `ffmpeg`, and `ffprobe` paths at startup, falls back to the system `PATH` when a configured path does not exist, and logs a clear error that names the variable to set when the tool is not found at all — see [Self Install](../getting-started/02-installation/self-install.md#step-6-create-the-env-configuration-file).
...
- Trailarr logs a warning when a profile's yt-dlp extra options override an option Trailarr sets (for example `-f`); the profile value stays active — see [Yt-dlp Extra Options](../user-guide/settings/profiles/settings/search.md#yt-dlp-extra-options).

---

## **v0.11.0** - _August 9, 2026_

**What's New:** ✨

- **The legacy status fields are gone for good** — Trailarr removed the internal `trailer_exists` and stored `status` database fields, so your download records are now the only source of truth — see the [roadmap](../references/roadmap.md).
...
- **Pick a local address for your Plex server** — the server-selection step after Plex sign-in now offers a **Custom URL** choice and a **Test** button, so you can point Trailarr at a local address (instead of the external URLs Plex provides) and check that it is reachable before you connect — see [Plex Connection](../getting-started/03-setup/plex-connection.md).
...
- The `Monitor Interval` setting is removed from Settings > General and from the settings API — it has had no effect since `v0.8.0`; change each task's interval on the [Tasks](../user-guide/tasks/index.md) page instead.

- You cannot downgrade below `v0.11.0` after upgrading. To go back, restore the automatic pre-upgrade backup — see [Backup & Restore](../troubleshooting/backup-restore.md).
