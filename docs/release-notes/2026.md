...
- **Connection Doctor** — every connection card now shows a health chip; the doctor checks that Trailarr can reach the application, see each reported media folder, and write to it — and when a folder is not visible, it suggests the exact path mapping to fix it, with a one-click **Apply** — see [Connection Doctor](../user-guide/settings/connections/index.md#connection-doctor).

- **Permission problems now name the exact fix** — a failed write test reports the folder's owner (uid/gid) against the user Trailarr runs as, so PUID/PGID mismatches are caught at setup time instead of failing three tasks later.

- **Dead network mounts are flagged, not passed** — a media folder that is reachable but empty is reported as a warning ("your mount may be down") instead of counting as healthy.
...
- **Find your media folders while you add a connection** — the doctor now runs on the Add / Edit Connection page too. Click **Find folders** and Trailarr looks on disk for the folders your Arr reports, then fills in each Trailarr Path it finds. A path you typed yourself is never overwritten — you get a button instead. This matters because Trailarr refuses to save a connection it cannot read, so before this you had to know your mount layout in advance — see [Connection Doctor](../user-guide/settings/connections/index.md#find-your-folders-while-you-add-a-connection).

- **A full media disk is now reported** — the Disk space check warns for every media disk below 5 GB, not only the config volume, and reports each disk of a split library.

- **New Settings > Health page** — live checks of FFmpeg, hardware acceleration (finally visible!), yt-dlp, cookies, connections, the image cache, and disk space; every red state names its fix — see [Health](../user-guide/settings/health.md).

- **Set up YouTube cookies from the UI** — upload or paste your cookies.txt on the Health page; Trailarr stores it with safe permissions, shows whether the cookies are valid or expired, and never displays the content again — see [YouTube cookies](../user-guide/settings/health.md#youtube-cookies).

- **Test YouTube downloads with one click** — the Health page can run a yt-dlp test against a known video (with your confirmation — it contacts YouTube once) and reports the result in plain language.

**Bug Fixes:** 🐛

- **Modals show again in Safari** — a dialog stayed hidden while the page behind it dimmed, because the dialog only became visible if its opening animation ran, and that animation does not run in Safari. A dialog now shows even when its animation does not run ([#653](https://github.com/nandyalu/trailarr/issues/653)).

- **Login works again behind a reverse proxy sub-directory** — the login returned a `405 Method Not Allowed` error, or was successful and then gave a `401 Unauthorized` error for every request, which locked you out of the app; the session cookie now stays at the root path, the redirect after login keeps the URL base, and a new `URL Base` applies without a restart — see [Reverse Proxy](../user-guide/reverse-proxy.md#login-problems-in-a-sub-directory-setup) ([#663](https://github.com/nandyalu/trailarr/issues/663)).

- **Your environment variables are used again** — a setting stored in the `.env` file replaced the variable you set in `docker-compose.yml`, so `URL_BASE=` and `WEBUI_PASSWORD=` did nothing and you could not correct a setting that locked you out; a variable set for the container now wins — see [Environment Variables](../getting-started/01-first-things/environment-variables.md) ([#663](https://github.com/nandyalu/trailarr/issues/663)).

**Other Changes:** ⚙️

- `WEBUI_PASSWORD` with a value that is not empty now sets that password (Trailarr stores only its hash) instead of breaking the login, and an empty value still resets the password to `trailarr` — see [WEBUI_PASSWORD](../getting-started/01-first-things/environment-variables.md#webui_password).

- Known yt-dlp failures are now recorded as a plain-language reason with the fix (for example, "YouTube requires a sign-in for this download. Set up a cookies file on Settings > Health.") instead of a raw traceback — see [Classified download errors](../user-guide/settings/health.md#classified-download-errors).

- Health check results are kept when Trailarr restarts, so connection chips and the Health page no longer reset to "not checked" after every container update.

- The Health page runs its checks together instead of one after another, so a hung mount delays only its own check.

- The Connections health check runs the Connection Doctor itself when nothing was checked yet, instead of asking you to go and run it.
