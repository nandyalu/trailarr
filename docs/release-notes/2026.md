...
- **The image refresh and the library sync work again** — both tasks stopped with an error as soon as they reached a media item, because a log line used a feature of a logger that this file did not have. The Image Refresh task failed every run, and the Arr Data Refresh failed at the end, where it refreshes images.

**Other Changes:** ⚙️

- **The YouTube download test no longer fails for everyone** — the test asked YouTube for one particular video, and that video was deleted, so the Health page reported that every setup was broken. Trailarr now asks for a search result, which always exists. A video that cannot be read is reported as a warning that says so, instead of an error about your setup — see [Health](../user-guide/settings/health.md#youtube-download-test).
