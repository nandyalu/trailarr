...
- **Modals show again in Safari** — a dialog stayed hidden while the page behind it dimmed, because the dialog only became visible if its opening animation ran, and that animation does not run in Safari. A dialog now shows even when its animation does not run. Thanks [@carlosjfcasero](https://github.com/carlosjfcasero) for the report! ([#653](https://github.com/nandyalu/trailarr/issues/653))
