...
- **Downloaded trailers show in Plex immediately again** — the `Notify Plex` profile setting scanned the media folder, but Plex kept the new trailer hidden on the media item until its metadata refreshed. Trailarr now refreshes the item too, so the trailer shows in seconds — see [Notify Plex](../user-guide/settings/profiles/settings/plex.md#notify-plex). ([#652](https://github.com/nandyalu/trailarr/pull/652), thanks [@lems111](https://github.com/lems111)!)
