## **v0.9.6-beta** - _June 04, 2026_

**Breaking Changes:** ⚠️

- **WEBUI_DISABLE_AUTH** — The `WEBUI_DISABLE_AUTH` setting has been removed. If you want to bypass the login screen (e.g. when your reverse proxy handles authentication), configure your reverse proxy to forward the `X-API-KEY` header with your Trailarr API key on every request. Trailarr will accept it and issue a session automatically. See the updated [Reverse Proxy Configuration](../user-guide/reverse-proxy.md) docs for examples.
