## **v0.10.0-beta** - _June 30, 2026_

**What's New:** ✨

- **Per-Profile Trailer Tracking** — Trailarr now tracks trailer downloads independently for each Trailer Profile applied to a media item. Previously, a single `trailer_exists` flag covered all profiles; now every `(media, profile)` pair has its own status (`Pending`, `Downloading`, `Downloaded`, `Not Available`, `Failed`, `Skipped`, `Unmonitored`). The media page shows the individual status of each profile for a media item. `Not Available` is set when TMDB is queried and has no matching video yet — it is automatically reset to `Pending` on the next weekly TMDB refresh once the video appears.
...
- **Video Type selection on Trailer Profiles** — Each profile now has a **Video Type** setting that controls what kind of video is searched for and downloaded: `Trailer`, `Teaser`, `Clip`, `Behind the Scenes`, `Bloopers`, `Featurette`, `Opening Credits`, or `Other`. Defaults to `Trailer`; existing profiles are unchanged. `Trailer` and `Other` always use YouTube search. All other types use the TMDB API and require a TMDB API Key — without one they are hidden in the profile editor.
...
- **TMDB API Key setting** — A new **TMDB API Key** field is available in General Settings (and as the `TMDB_API_KEY` environment variable). Required for all non-trailer, non-other Video Types (Teaser, Featurette, etc.) and for the **TMDB Language** profile setting. Without it, only `Trailer` and `Other` video types are available in the profile editor.
- **TMDB Language on Trailer Profiles** — Each profile has a new optional **TMDB Language** field (ISO 639-1 code, e.g. `de`, `fr`). When set, Trailarr queries TMDB in that language first before falling back to the Radarr-provided ID and YouTube search. For TMDB video types (Featurette, Teaser, etc.) it controls the language of every TMDB lookup. Leave empty to use the TMDB default (en-US). Requires a TMDB API Key.
- **TMDB video key caching** — The **Refresh TMDB Videos** weekly task now caches the YouTube key returned by TMDB directly on each status row. The download task reads this cache first, skipping a redundant live TMDB call on every download run. TMDB-sourced keys are stored only on the status row and never overwrite the Radarr-provided YouTube ID on the media item.
...
> - **`Stop Monitoring` behaviour change.** When a profile with *Stop Monitoring* enabled successfully downloads a trailer, it now **also** marks all other Pending profile rows for that media item as Skipped (across every profile), so other profiles do not attempt a redundant download for the same item. The media's Monitor flag is still turned off as before. If you have TMDB-type profiles (Featurette, Teaser, etc.) alongside a trailer profile, set `Stop Monitoring = false` on the trailer profile so TMDB rows are not skipped before TMDB has had a chance to find the video.
...
- **Trailer re-download loop on large files** — The file scanner was not recognising downloaded trailers larger than 100 MB, which caused a repeated re-download loop: the scanner missed the file, silently reset `trailer_exists` to false, the connection refresh re-enabled monitoring, and the download task picked it up again. Fixed by: (1) files placed inside a recognised trailer folder (e.g. `Trailers/`) are now always detected as trailers regardless of size; (2) inline files with "trailer" in the name up to 200 MB pass without extra checks; (3) inline files over 200 MB are verified via ffprobe — if their duration is ≤ 10 minutes they are treated as trailers.
...
- **Monitor flag re-enabled on every connection refresh** — Media items manually set to unmonitored were silently having their monitor flag reset to `true` on every Arr or Plex refresh when the connection's monitoring mode was set to *Monitor Missing*. The monitor flag is now preserved for existing items: once set to `false`, only *Sync* mode (which explicitly mirrors the Arr monitored flag) can re-enable it.
