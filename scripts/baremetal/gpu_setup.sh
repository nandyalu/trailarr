    ...
    if [ "$GPU_AVAILABLE_NVIDIA" = "true" ]; then
        ...
        if [ -n "$GPU_INFO" ]; then
            ...
            if [ -n "$GPU_DEVICE_NVIDIA" ]; then
                show_message $GREEN "NVIDIA GPU device: $GPU_DEVICE_NVIDIA"
            fi
        fi
        ...
        if [ -n "$NVIDIA_GPU" ]; then
            ...
            show_message $YELLOW ">>>    sudo apt update && sudo apt install -y nvidia-driver"
        ...
        show_message "No NVIDIA GPU detected"
    fi
}
...
check_intel_gpu() {    
    ...
    if [ "$GPU_AVAILABLE_INTEL" = "true" ]; then
        ...
        if [ -n "$INTEL_GPU" ]; then
            ...
            show_message $GREEN "Intel GPU device: $GPU_DEVICE_INTEL"
            DETECTED_GPUS+=("Intel")
            ...
            if command -v vainfo &> /dev/null; then
                ...
                if [ -n "$VAAPI_INFO" ]; then
                    show_message $GREEN "Intel VAAPI capabilities detected"
                fi
            else
        ...
        if [ -d /dev/dri ]; then
            show_message "No Intel GPU detected in DRI devices"
        else
            show_message "Intel GPU not available - /dev/dri does not exist"
        fi
    fi
}
...
check_amd_gpu() {    
    ...
    if [ "$GPU_AVAILABLE_AMD" = "true" ]; then
        ...
        if [ -n "$AMD_GPU" ]; then
            ...
            show_message $GREEN "AMD GPU device: $GPU_DEVICE_AMD"
            DETECTED_GPUS+=("AMD")
            AVAILABLE_GPUS+=("amd")
            
            # Test VAAPI capability for AMD
            if command -v vainfo &> /dev/null; then
                VAAPI_INFO=$(vainfo --display drm --device "$GPU_DEVICE_AMD" 2>/dev/null | grep -i "VAProfile" | grep -iE "H264|HEVC|VP8|VP9|AV1")
                if [ -n "$VAAPI_INFO" ]; then
                    show_message $GREEN "VAAPI capabilities detected"
                fi
            else
        ...
        if [ -d /dev/dri ]; then
            show_message "No AMD GPU detected in DRI devices"
        else
            show_message "AMD GPU not available - /dev/dri does not exist"
        fi
    fi
}
...
save_gpu_env_vars() {
    ...
    show_message $GREEN "GPU configuration saveded successfully"
}
...
main() {
    ...
    show_message "GPU hardware detection complete"
}
