...
class TestVideoConversionHardwareAcceleration:
    ...
    @patch.dict(
        os.environ,
        {
            "GPU_DEVICE_INTEL": "/dev/dri/renderD129",
        },
        clear=True,
    )
    ...
    def test_unsupported_codec_fallback_vaapi(self, mock_cpu, temp_test_file):
        ...
        mock_cpu.assert_called_once_with(
            "unsupported_codec", temp_test_file, None
        )
    ...
    def test_ffmpeg_cmd_intel_gpu(
        ...
        # Generate command
        cmd = get_ffmpeg_cmd(
            trailer_profile, temp_input_file, temp_output_file
        )
    ...
    def test_ffmpeg_cmd_fallback_mode(
        ...
        cmd = get_ffmpeg_cmd(
            trailer_profile, temp_input_file, temp_output_file, fallback=True
        )
