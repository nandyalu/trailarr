...
from core.base.database.models.downloadattempt import DownloadAttemptRead
from core.base.database.models.media import MediaRead
from core.base.utils.satisfaction import SatisfactionResult
from core.download.trailers.missing import (
...
def sweep_harness():
    ...
    with ExitStack() as stack:
        settings = stack.enter_context(
            patch("core.download.trailers.missing.app_settings")
        )
        stack.enter_context(
            patch(
        ...
        attempts = stack.enter_context(
            patch("core.download.trailers.missing.attempt_manager")
        )
        ...
        profiles = stack.enter_context(
            patch("core.download.trailers.missing.trailerprofile")
        )
...
async def test_narrowed_profile_filter_drops_later_media(sweep_harness):
    ...
    sweep_harness.process.side_effect = process
    with patch(
        "core.download.trailers.missing.find_matching_profiles",
