"""Tests for services/auth.py — sessions, hashing and credential checks.

These moved out of tests/api/test_authentication.py in Phase 7 Stage B, with
the code they cover. The FastAPI dependencies that map these answers to 401
responses are still tested in that file.
"""
...
class TestGetSession:
    ...
    def test_skips_invalid_token_and_creates_new(self):
        ...
        create_session()
        create_session()
        with patch("services.auth.verify_session", side_effect=[False, True]):
            token = get_session()
        assert token in auth_module._sessions
