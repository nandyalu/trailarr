...
def make_download(
    id: int, path: str, file_exists: bool = True
) -> SimpleNamespace:
...
class TestDeleteFileFolDeletionFailure:
    ...
    async def test_deletion_failure_skips_db_updates(self):
        with (
            patch(
                "services.files.service.FilesHandler.delete_file_fol",
            ...
            patch(
                "services.files.service.download_manager.mark_as_deleted"
            ) as mock_mark,
        ):
