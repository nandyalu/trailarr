...
def _manager(path_mappings: list, all_path_mappings: list | None = None):
    ...
    return SimpleNamespace(
        ...
        all_path_mappings=(
            all_path_mappings
            if all_path_mappings is not None
            else path_mappings
        ),
    )
...
class TestApplyPathMapping:
    ...
    def test_linux_to_linux(self):
        ...
        result = PlexConnectionManager._apply_path_mapping(
            mgr, "/mnt/plex/movies/Film (2020)"
        )
    ...
    def test_linux_to_linux_nested(self):
        ...
        result = PlexConnectionManager._apply_path_mapping(
            mgr, "/mnt/plex/movies/Film/extras"
        )
    ...
    def test_linux_to_linux_no_false_positive(self):
        mgr = _manager([_pm("/mnt/plex/movies", "/local/movies")])
        result = PlexConnectionManager._apply_path_mapping(
            mgr, "/mnt/plex/movies-backup/Film"
        )
    ...
    def test_macos_to_linux(self):
        ...
        result = PlexConnectionManager._apply_path_mapping(
            mgr, "/Volumes/Plex/Movies/Film (2020)"
        )
    ...
    def test_macos_to_linux_no_false_positive(self):
        mgr = _manager([_pm("/Volumes/Plex/Movies", "/data/movies")])
        result = PlexConnectionManager._apply_path_mapping(
            mgr, "/Volumes/Plex/Movies-Archive/Film"
        )
    ...
    def test_backslashes_normalised_even_without_mapping(self):
        mgr = _manager([])
        result = PlexConnectionManager._apply_path_mapping(
            mgr, "some\\path\\with\\backslashes"
        )
...
class TestReversePathMapping:
    ...
    def test_linux_to_linux_reverse(self):
        mgr = _manager([_pm("/mnt/plex/movies", "/local/movies")])
        result = PlexConnectionManager._reverse_path_mapping(
            mgr, "/local/movies/Film (2020)"
        )
...
class TestIsInConfiguredLibrary:
    ...
    def test_linux_match(self):
        mgr = _manager(
            [], all_path_mappings=[_pm("/mnt/plex/movies", "/local/movies")]
        )
        assert (
            PlexConnectionManager._is_in_configured_library(
                mgr, "/mnt/plex/movies/Film"
            )
            is True
        )
    ...
    def test_windows_match_backslash(self):
        mgr = _manager(
            [], all_path_mappings=[_pm("C:\\Plex\\Movies", "/local/movies")]
        )
        assert (
            PlexConnectionManager._is_in_configured_library(
                mgr, "C:\\Plex\\Movies\\Film"
            )
            is True
        )
    ...
    def test_macos_match(self):
        mgr = _manager(
            [], all_path_mappings=[_pm("/Volumes/Plex/Movies", "/data/movies")]
        )
        assert (
            PlexConnectionManager._is_in_configured_library(
                mgr, "/Volumes/Plex/Movies/Film"
            )
            is True
        )
    ...
    def test_macos_boundary_false_positive(self):
        ...
        assert (
            PlexConnectionManager._is_in_configured_library(
                mgr, "/Volumes/Plex/Movies-old/Film"
            )
            is False
        )
...
class TestSectionIsTracked:
    ...
    def test_single_folder_matches(self):
        mgr = _manager(
            [], all_path_mappings=[_pm("/mnt/plex/movies", "/local")]
        )
    ...
    def test_windows_folder_matches(self):
        mgr = _manager(
            [], all_path_mappings=[_pm("C:\\Plex\\Movies", "/local")]
        )
    ...
    def test_macos_folder_matches(self):
        mgr = _manager(
            [], all_path_mappings=[_pm("/Volumes/Plex/Movies", "/data")]
        )
