...
async def check_for_update(check_app: bool = True) -> None:
    ...
    if current_version != latest_version:
        logger.info(f"A newer version ({latest_version}) of {_app} is available. Please update!")
        if check_app:
            app_settings.update_available = True
        else:
            app_settings.update_available_ytdlp = True
    else:
        logger.info(f"You are using the latest version ({latest_version}) of {_app}.")


async def check_for_updates() -> None:
    await check_for_update(True)
    await check_for_update(False)
