        ...
        async with session.get(url, params=params, timeout=_REQUEST_TIMEOUT) as resp:
            ...
            if resp.status != 200:
                logger.warning(
                    ...
                    f"{'movie' if is_movie else 'tv'}/{tmdb_id} (season={season})"
                )
                return []
            data = await resp.json()
            return data.get("results", [])
    except asyncio.TimeoutError:
        logger.warning(f"TMDB request timed out for {'movie' if is_movie else 'tv'}/{tmdb_id}")
        return []
...
def _find_youtube_key(results: list[dict], video_type: str) -> str | None:
    ...
    for r in results:
        if r.get("site", "").lower() == "youtube" and (r.get("type") or "").lower() == target:
            return r.get("key")
    return None


async def refresh_tmdb_videos(_stop_event: threading.Event | None = None) -> None:
    ...
    groups: dict[tuple[int, int], list[TmdbPendingRow]] = defaultdict(list)
    for row in rows:
    ...
    async with aiohttp.ClientSession() as http_session:
        for (media_id, season), group_rows in groups.items():
            ...
            if not media.txdb_id:
                logger.debug(f"Media {media_id} has no txdb_id — skipping TMDB check.")
                continue

            try:
                results = await _fetch_tmdb_videos(
                    http_session, media.txdb_id, media.is_movie, season, api_key
                )
            except Exception as exc:
                logger.error(f"Unexpected error fetching TMDB videos for media {media_id}: {exc}")
                errors += 1
                continue
            ...
            for row in group_rows:
                ...
                if row.video_type == VideoType.TRAILER:
                    if yt_key and row.status == TrailerStatusEnum.PENDING:
                        media_repo.update_ytid(media_id, yt_key)
                    continue

                if has_result and row.status == TrailerStatusEnum.NOT_AVAILABLE:
                    trailer_status_repo.update_row_status(row.row_id, TrailerStatusEnum.PENDING)
                    logger.debug(
                    ...
                    trailer_status_repo.update_row_status(row.row_id, TrailerStatusEnum.NOT_AVAILABLE)
                    logger.debug(
